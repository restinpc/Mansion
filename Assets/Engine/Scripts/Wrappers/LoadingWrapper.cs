...
namespace Engine.Wrappers
{
    ...
    {
        public LoadingWrapper(string objectName, Dictionary<string, Components.Component> level, Scene scene, Components.Component parent = null)
        {
            Dictionary<string, Prop> mapStateToProps(Dictionary<string, Prop> state)
            {
                ...
                {
                    { "visible", new Prop(
                        scene.ToString() == state["activeScene"].getString()
                        && (state["loading"].getBool() || !state["started"].getBool())
                    )}
                };
            }
